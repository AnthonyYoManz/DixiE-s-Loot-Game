...
void Player::handleEvents(){
    if(actionMap != NULL){
        if(game->m_input.getInputActive((*actionMap)[PLAYER_RIGHT])){
            cVelocity.x = maxMoveSpeed;
        }
        else if(game->m_input.getInputActive((*actionMap)[PLAYER_LEFT])){
            cVelocity.x = -maxMoveSpeed;
        }
        else{
            cVelocity.x = 0;
        }
        if(game->m_input.getInputActive((*actionMap)[PLAYER_DOWN])){
            cVelocity.y = maxMoveSpeed;
        }
        else if(game->m_input.getInputActive((*actionMap)[PLAYER_UP])){
            cVelocity.y = -maxMoveSpeed;
        }
        else{
            cVelocity.y = 0;
        }
        if(game->m_input.getInputActive((*actionMap)[PLAYER_ACTIVATE_R1])){
            if(rightHandItem != NULL){
                rightHandItem->activate(true);
            }
        }
        if(game->m_input.getInputActive((*actionMap)[PLAYER_ACTIVATE_R2])){
            if(rightHandItem != NULL){
                rightHandItem->activate(false);
            }
        }
    }
    lookAt(sf::Vector2f(sf::Mouse::getPosition(game->m_window)) + scene->getCameraPosition());
